...
(function(window, document, $) {
  ...
  $.site.menu = {
      ...
      switch (flag) {
        case 'on':
          $(document).on('click.site.menu', function(e) {
            if ($('.dropdown > [dropdown-tag="true"]').length > 0) {
              if ($(e.target).closest(".dropdown-menu").length === 0) {
                $('.dropdown > [dropdown-tag="true"]').attr('dropdown-tag', 'false').closest(".dropdown").removeClass('open');
              }
            }
          });
          break;
    ...
    open: function($tag) {
      if ($tag.is('.dropdown')) {
        $('[dropdown-tag="true"]').attr('dropdown-tag', 'false').closest(".dropdown").removeClass('open');
        $tag.find('>.dropdown-toggle').attr('dropdown-tag', 'true');
      }
      $tag.addClass('open');
    },
    ...
    close: function($tag) {
      ...
      if ($tag.is('.dropdown')) {
        $tag.find('>.dropdown-toggle').attr('dropdown-tag', 'false');
      }
    },
